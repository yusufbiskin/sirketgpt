...
def olustur():
    ...
    for o in ortaklar:
    doc.add_paragraph(
        f"-Beheri {o['pay_tutari']:,.2f} Türk Lirası degerinde {o['pay_adedi']} adet paya karsılık gelen "
        f"{o['sermaye']:,.2f} Türk Lirası {o['ad'].upper()} tarafından nakdi olarak taahhüt edilmistir."
    )
...
doc.add_paragraph("Nakden taahhüt edilen payların itibari degerleri, sirketin tescilini izleyen 24 ay içinde ödenecektir.")
    ...
    doc.add_paragraph("Sirketi müdürler temsil ederler. Sirketi temsil edecek imzalar genel kurul tarafından tespit, tescil ve ilan olunur. Müdürler, sirkete hizmet akdi ile baglı olanları sınırlı yetkiye sahip
    ticari vekil veya diger tacir yardımcıları olarak atayabilir. Bu sekilde atanacak olanların görev ve yetkileri, hazırlanacak iç yönergede açıkça belirlenir. Bu durumda iç yönergenin
    tescil ve ilanı zorunludur. Iç yönerge ile ticari vekil ve diger tacir yardımcıları atanamaz. Yetkilendirilen ticari vekil veya diger tacir yardımcıları da ticaret siciline tescil ve ilan edilir.
    ...
    doc.add_paragraph("Genel Kurullar, olagan ve olaganüstü toplanırlar. Olagan genel kurul, her yıl hesap döneminin sona ermesinden itibaren 3 ay içinde; olaganüstü genel kurullar ise, Sirket islerinin
    ...
    doc.add_paragraph("Genel kurulun toplantıya çagrılmasına iliskin ilanlar da dahil olmak üzere Sirkete ait ilanlar Türkiye Ticaret Sicili Gazetesinde yapılır. Genel kurul toplantılarına iliskin ilanların
    toplantı gününden en az on gün önce yapılması zorunludur.")

    doc.add_heading('11. HESAP DÖNEMİ', level=1)
    doc.add_paragraph("Sirketin hesap yılı, Ocak ayının 1. gününden baslar ve Aralık ayının 31. günü sona erer. Fakat birinci hesap yılı, Sirketin kesin olarak kuruldugu tarihten itibaren baslar ve o
    senenin aralık ayının otuz birinci günü sona erer.")

    doc.add_heading('12. KARIN TESPİTİ VE DAĞITIMI', level=1)
    doc.add_paragraph("Sirketin net dönem karı yapılmıs her çesit masrafların çıkarılmasından sonra kalan miktardır. Net dönem kârından her yıl %5 genel kanuni yedek akçe ayrılır; kalan miktar, genel
